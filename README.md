...
6. **Open Inventory**  
Prompts the user to select a character. Input the number of a character to print a list of their inventory.
7. **Add Item**  
...
8. **Remove Item**  
Prompts the user to select a character. Selecting a character will print a list of the character's inventory. Input the corresponding number to remove an item.
9. **Quit**  
...
This is the last project I will make with this menu loop style. It was kind of getting old after the contact book. Originally, the scope of this project was just going to be exploring nested structs, but I ended up branching out into a lot of different concepts. This is by far the most robust thing I have built thus far. I am not happy with some of the design choices, and inconsistency regarding the menu. However, I am happy with a lot of the structural choices I made for the project, which made it easier to understand in the end.

### Pointers

Pointers seem to be a recurring topic. I don't want to go to into memory allocation yet, but oh boy the limitations in structs. Originally, the character struct had inventory as an array. This has big problems though as it turns out, because the character struct is used in an array of character structs called party. C does not like this my compiler told me. Now if character wasn't in an array, I believe putting inventory as an array at the end of character would have been fine.

So anyway, back to the pointer bit because it's important here. Instead of shoving the dangerous array into the character struct we just made a pointer. Now this is honestly what broke my brain for a bit due to how arrays normally work. For some reason in my head, I just assumed passing a pointer meant you were directly changing the values at a memory address, and this was enough. Well, it turns out functions like `realloc()` spit out a pointer. Well, if we want to change a pointer we passed as an argument we need to go another layer deeper. Behold `**ptr` in all its glory. A memory address that holds a memory address. Now when we pass this, we can dereference it which will allow us to access the memory address of our dynamic array not the local version in the function.

This introduces a lot of syntax issues that basically can be solved by wrapping the dereference in parenthesis like `(*ptr)[index]`. Essentially this is what locks in the idea that an array is just a memory address, and in this case, we need to keep track of its size manually.

###  Multiple Files

This is the first project I decided to have multiple files. I thought this would help me keep my sanity. Well, it kind of ended up doing the opposite for a bit. Honestly, I don't even know where to begin. This entire process began with me manually copying segments of my code over to individual files which I won't name because they changed several times over the course of the project. I will mention though that this was a learning process where I had to think more deeply about specific roles.

I guess we can start to talk about the problems. File dependency is not something to mess with in C. I'm going to be completely honest here; I had issues I didn't understand and fixed them by simply moving things around. I believe what was happening is basically a header file would need a specific type for let's say character, and originally, I had the character struct in the character header. Well turns out the character header also needs the one calling it creating a loop where something isn't initialized properly creating confusing errors. The simple solution to this is basically to just have a `"types.h"` header which can be used instead of multiple headers. Another solution seemed to be thoroughly analyzing what each file needed carefully and minimizing the includes. Using lighter headers like `<stddef.h>` if you only need things like `size_t` and `NULL`.

Outside of the dependencies it's probably worth mentioning the `#ifndef` macro. This will basically only do what is in its body which is ended with `#endif` if what you input after isn't defined. This seems to only check when the preprocessor reads the line, so if you put `#define` on the line after you can make it so that the file won't be read again but will be read this time. This is important because if you include the same header multiple times in your project everything in it like types will get redefined which is bad.

Okay time to talk about an external tool that made all this possible. Make basically lets you automate the building process. You create a file named a Makefile which the `make` command will read and run. I'm not going to explain specifics about the syntax, but essentially you give a build target then list requirements for that target. On the line below you enter the console commands to obtain that build target. If there are requirements listed that are missing and there's targets for them, it'll follow those requirements and commands to build them. Make will only build targets that have requirements that are newer than the build target. It's also worth noting GCC has compiler flags to generate .d files which can be `-include` anywhere in your Makefile to have headers added on as dependencies.

### Input Validation

I became very familiar with the way `fgets()` functions in this project as I stopped using `scanf()` entirely. `fgets()` has a lot of issues revolving around the way it deals with the stdin buffer. I created a robust function for generating error codes and properly filtering these inputs including a variation for integers.

The core thing to remember with `fgets()` is that if you enter one character you need a buffer size of 3 bytes to store everything `fgets()` outputs. The reason for this is that in addition to your character `fgets()` will insert a new line character `\n` and terminator `\0`. The terminator must exist in a string to signify the end of it. However, in the case you have a buffer size of 2 bytes, the new line character does not need to exist, but this is where a "bug" if you want to call it that exists. The new line character is not simply deleted. Instead, it is pushed back into the stdin buffer for `fgets()` to pull again. What will happen is the next time `fgets()` is called it will immediately exit eating the input, because `\n` is the same as hitting enter. This is intended behavior and is why cleaning the stdin buffer is important. In my code I did this by checking if the string held `\n` to decide whether this needed to happen.
...
Now most of the project revolves around numerical input, and this is simply something I would probably never do again because it feels horrible from a user perspective. Instead, I would have the user type commands, with maybe some advanced form of fuzzy matching I'd have to learn how to make. I did make a variation of the above function that utilizes `atoi()`. In the future I would probably explore other methods like `strtol()` because I had to utilize guard clauses to account for a lot of scenarios `atoi()` couldn't handle.

To manage the menu inputs and validate whether the input was valid for that menu I had to create a validation function that was flexible enough to be reused. This function would basically filter the status of the `clean_input_int()` and normalize user input for menu options. This would return a status that was simpler than `clean_input_int()` and would tell the menu whether to proceed in a switch statement or do other things like reading error messages.
...
With this being in mind you the programmer sees text, but the actual machine sees numbers. This makes enums very efficient for things like switch statements. It is worth noting however enums can only represent integers, if you need a float like PI at compile time your only choice is `#define`. In general, I would avoid using `#define` and instead use constant variables if you need different types of constants. The reason for this is that `#define` has no type safety, but actual variables do, and they also respect scope. It is very important though to note that enums and `#define` are not variables, they are replacements the compiler does.
